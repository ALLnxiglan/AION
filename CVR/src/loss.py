...
def aion_loss(targets, outputs, params=None):
    z = targets["label"]
    x = outputs["logits"]
    x = tf.reshape(x, (-1,))
    z = tf.cast(z, tf.float32)
    ...
    if isinstance(outputs, dict) and "pre_logits" in outputs:
        pre_x = outputs["pre_logits"]
        pre_x = tf.reshape(pre_x, (-1,))
        p_no_grad = tf.sigmoid(tf.stop_gradient(pre_x))
    else:
        p_no_grad = tf.sigmoid(tf.stop_gradient(x))

    weight = tf.where(tf.equal(z, 1.0), tf.ones_like(z) * E_y, E_y * (1.0 - p_no_grad))

    pos_loss = stable_log1pex(x)
    neg_loss = stable_log1pex(-x)
    base_loss = tf.where(tf.equal(z, 1.0), pos_loss, neg_loss)
...
def unbiased_fnw_loss(targets, outputs, params=None):
    ...
    p_no_grad = tf.sigmoid(tf.stop_gradient(x))
    wi = p_no_grad
    loss1 = stable_log1pex(x)
    loss2 = x + stable_log1pex(x)
    loss3 = stable_log1pex(x)
    loss1_weight = one
    ...
    loss = tf.reduce_mean(z * loss1 + (one - z) * (loss2 + loss3))
    return {
        "loss": loss
    }
...
def delay_tn_dp_loss(targets, outputs, params=None):
    ...
    tn_loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=tn_label, logits=tn)*tn_mask) / tf.reduce_sum(tn_mask)
    dp_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=dp_label, logits=dp))
...
def inw_outw_cross_entropy_loss(targets, outputs, params=None):
    ...
    inw_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=inw_label, logits=inw_logits))
    outw_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=outw_label, logits=outw_logits))
...
def unbiased_bidefuse_loss(targets, outputs, params=None):
    ...
    inw_loss = tf.reduce_sum((cvr_label * inw_pos + (1 - cvr_label) * inw_neg) * inw_mask) / tf.reduce_sum(inw_mask)
    one = tf.constant([1.])
    oloss = 'DEFUSE'
    if oloss == 'DEFUSE':
        ...
        loss2 = loss2_weight * loss2 * (one - wi)
        loss3 = loss3_weight * loss3 * wi
        loss4 = ((outw_label * loss1_weight) + (one - outw_label) * ((one - wi) * loss2_weight + wi * loss3_weight)) * tf.math.log(one + tf.math.sigmoid(outw_logits))
        outw_loss = tf.reduce_mean(outw_label * loss1 + (one - outw_label) * (loss2 + loss3))
    elif oloss == 'ce':
